  ...
  template: `
    <nav class="flex mb-8" aria-label="Breadcrumb">
      <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
        <li *ngFor="let item of items; let last = last; let i = index" class="inline-flex items-center">
          ...
          <ng-container *ngIf="item.route && !last">
            ...
            >
              <ng-icon *ngIf="i === 0" name="heroGlobeAlt" size="0.75rem" class="mr-2.5"></ng-icon>
              {{ item.label }}
            </a>
          </ng-container>
